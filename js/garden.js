...
Garden.prototype.fallingPetals = [];

Garden.prototype.updateFallingPetals = function () {
	var ctx = this.ctx;
	for (var i = this.fallingPetals.length - 1; i >= 0; i--) {
		var petal = this.fallingPetals[i];
		petal.update();
		petal.draw(ctx);
		if (petal.isOutOfCanvas(this.element.height)) {
			this.fallingPetals.splice(i, 1); // 移除画布外的花瓣
		}
	}
};